def find_button_envoyer_sans_note(driver):
    ...
    try:
        script_find_button_envoyer_sans_note = """
                                                function findButton() {
                                                    // 1. Check standard
                                                    let btn = dossier_competences.querySelector('button[aria-label="Envoyer sans note"]');
                                                    if (btn) return btn;
    
                                                    // 2. Check dans le Shadow DOM (interop-outlet)
                                                    let host = dossier_competences.querySelector('#interop-outlet');
                                                    if (host && host.shadowRoot) {
                                                        return host.shadowRoot.querySelector('button[aria-label="Envoyer sans note"]');
                                                    }
    
                                                    // 3. Check par texte si aria-label a sauté
                                                    return Array.from(dossier_competences.querySelectorAll('button')).find(b => b.innerText.includes('sans note'));
                                                }
