...
def build_section_competences(doc, data):
    ...
    comps = data.get('15_Compétences_Percutantes_Sous_Forme_De_Phrase') or data.get('Compétences') or []
    comps = [c for c in comps if c and c.strip()]
    if not comps: return

    header_section(doc, "DOMAINES DE COMPETENCES")

    for comp in (data.get('15_Compétences_Percutantes_Sous_Forme_De_Phrase') or data.get('Compétences') or []):
